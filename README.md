...
3. 하단 **Call Flow** 툴윈도우에 관계도가 표시됩니다.
   ...
   - **하단 범례의 타입**(Service/Repository/Util/API) 클릭 → 그 타입 노드 표시/숨김.
     잡음이 많은 **`Util` 은 기본 숨김**이며, 클릭하면 다시 표시됩니다.
...
- 기준 메소드에서 양방향 **2단계**까지 BFS, 노드당 최대 **14가지**
  (`CallGraphAnalyzer.MAX_DEPTH`, `MAX_CHILDREN` 으로 조절). 더 깊이 보려면
  해당 메소드에서 다시 **Show Call Flow** 를 실행하면 그 메소드가 새 중심이 됩니다.
...
- [ ] 노드에서 바로 새 중심으로 재분석하는 무한 드릴다운(이전 시도 제거됨, 재설계 후보)
